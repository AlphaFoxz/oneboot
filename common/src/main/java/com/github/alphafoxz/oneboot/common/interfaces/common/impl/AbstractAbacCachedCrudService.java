...
import com.github.alphafoxz.oneboot.common.interfaces.common.*;
...
public abstract class AbstractAbacCachedCrudService<TABLE extends TableImpl<RECORD>, PO extends java.lang.Record, RECORD extends UpdatableRecordImpl<RECORD>>
    ...
    private int insert(boolean useAc, @NonNull RECORD record) {
        if (useAc && !acApi.access(getCurrentSubjectId(), getTable().getSchema().getName(), getTable().getName(), null, AbacActionType.CREATE, getCreatePolicies())) {
    ...
    private PO selectOne(boolean useAc, long id) {
        if (useAc && !acApi.access(getCurrentSubjectId(), getTable().getSchema().getName(), getTable().getName(), id, AbacActionType.READ, getReadPolicies())) {
    ...
    private int selectCount(boolean useAc, @Nullable Condition... conditions) {
        if (useAc && !acApi.access(getCurrentSubjectId(), getTable().getSchema().getName(), getTable().getName(), null, AbacActionType.READ, getReadPolicies())) {
            throw403Error(getTable().getName(), null);
        }
    ...
    private int update(boolean useAc, @NonNull RECORD record) {
        Long id = record.get(getIdField());
        if (useAc && !acApi.access(getCurrentSubjectId(), getTable().getSchema().getName(), getTable().getName(), id, AbacActionType.UPDATE, getUpdatePolicies())) {
            throw403Error(getTable().getName(), id);
        }
        if (id == null) {
            return 0;
    ...
    private int deleteById(boolean useAc, long id) {
        if (useAc && !acApi.access(getCurrentSubjectId(), getTable().getSchema().getName(), getTable().getName(), id, AbacActionType.DELETE, getDeletePolicies())) {
            throw403Error(getTable().getName(), id);
        }
        evictCache(id);
    ...
    private int[] deleteByIds(boolean useAc, @NonNull RECORD... records) {
        if (useAc) {
            for (RECORD record : records) {
                Long id = record.get(getIdField());
                if (!acApi.access(getCurrentSubjectId(), getTable().getSchema().getName(), getTable().getName(), id, AbacActionType.DELETE, getDeletePolicies())) {
                    throw403Error(getTable().getName(), id);
                }
            }
        }
